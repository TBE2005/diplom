...
export function GoalTemplate(settings: Doc<"goals"> & { collected: number, total: number, name: string }) {
    return (
        <Card bg={settings.backgroundColor} h={150} pos="relative">
            <Paper w={`${settings.collected / settings.total * 100}%`} h={150} color={settings.indicatorColor} pos="absolute" top={0} left={0} />
